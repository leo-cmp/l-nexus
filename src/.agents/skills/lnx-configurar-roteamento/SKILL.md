...
1. `.ai/model-routing.yaml`:
   - Atualize `risk_domains.project` e `terminal_runners`. **Não** mexa em
     `combos`, `roles`, `routes`, `execution_policy`, `default_runner` nem
     `cli_runners`: esses são do kit e chegam por `sync-routing`; editá-los no
     projeto cria divergência silenciosa a cada atualização.
